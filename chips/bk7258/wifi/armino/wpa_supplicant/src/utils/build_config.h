...
/* BK7258 CP runtime is STA-only. Keep AP/P2P/WPS providers out of the
 * translation unit through the upstream feature guards. */
#undef CONFIG_AP
#undef CONFIG_P2P
#undef CONFIG_WPS
#undef CONFIG_WPS_AP
...
/* sys_config.h uses numeric feature values for the MAC glue.  The WPA
 * sources also use #ifdef for optional providers, so remove those names from
 * the preprocessor namespace in the STA-only translation units. */
#undef CONFIG_P2P
#undef CONFIG_WPS
#undef CONFIG_WIFI_VNET_CONTROLLER
